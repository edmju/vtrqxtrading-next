// path: src/app/(terminal)/dashboard/sentiment/SentimentClient.tsx
// (game‑changer UI conservant toutes les fonctionnalités; courbe fine)
// Source d'origine: :contentReference[oaicite:1]{index=1}
"use client";
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // on garde toutes les lignes qui ont un score global numérique
            ...
            <div className="group rounded-3xl p-6 bg-gradient-to-br from-sky-900/50 via-sky-900/15 to-sky-600/10 border border-sky-500/35 shadow-[0_0_35px_rgba(8,47,73,0.9)] backdrop-blur-2xl transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_0_55px_rgba(8,47,73,1)] hover:border-sky-400/70">
              ...
              <div className="mt-4 flex items-center gap-6">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-sky-100/80 leading-snug">IA : {(marketRegime?.description || "").split(/[.\n]/)[0] || "lecture neutre."}</p>
                </div>
              </div>
                ...
                {(["24h", "72h", "7j", "all"] as const).map((k) => (
                  <button
                    ...
                    className={`px-2 py-1 rounded-full border transition ${
                      windowSel === k ? "border-emerald-500/60 text-emerald-300 bg-emerald-500/10" : "border-neutral-700/70 text-neutral-300 hover:border-neutral-500/70"
                    }`}
                  >
                    {k === "all" ? "Tout" : k}
                  </button>
