...
function fallbackDeterministic(articles: RawArticle[], ftmoPool: string[], topThemes: number): AiOutputs {
  ...
  const themes: AiTheme[] = sigs
    ...
    .map(s => ({
      ...
      summary: s.label,
      evidenceIds: s.evidences.map(e=>e.id ?? "").filter(Boolean).slice(0,5)
    }));

  // clusters simples par mot-clé du label de thème
  const clusters: AiCluster[] = themes.map(th => {
    const key = th.label.toLowerCase().split(/[()\s/,&-]+/).filter(Boolean).slice(0,3);
    const ids = articles
      .filter(a => key.some(k => (a.title + " " + (a.description||"")).toLowerCase().includes(k)))
      .slice(0, 20)
      .map(a => a.id ?? "")
      .filter(Boolean);
    return { label: th.label, weight: th.weight, summary: th.summary || th.label, articleIds: ids };
  });
  ...
  for (const s of sigs.sort((a,b)=>b.strength-a.strength)) {
    ...
    if (s.key === "dovish_us" && s.strength >= 0.35) {
      ...
      out.push({ symbol: pool.includes("EURUSD") ? "EURUSD" : pool[0], direction:"BUY", conviction: 5, confidence: conf, reason:"USD susceptible de se détendre", evidenceIds: s.evidences.map(e=>e.id!).slice(0,3) });
    }
  ...
  return { generatedAt: new Date().toISOString(), mainThemes: themes, actions: out, clusters };
}
...
- Réponds SEULEMENT au format JSON: {
  ...
  try {
    ...
    try {
      ...
      const themes: AiTheme[] = Array.isArray(data.mainThemes) ? data.mainThemes.slice(0, topThemes) : [];
      const clusters: AiCluster[] = Array.isArray(data.clusters) ? data.clusters.slice(0, 6) : [];
      const actions: AiAction[] = Array.isArray(data.actions) ? data.actions.slice(0, 4).map((a: any) => ({
        ...
        evidenceIds: Array.isArray(a.evidenceIds) ? a.evidenceIds.slice(0,12).map(String) : []
      })) : [];

      out = { generatedAt: new Date().toISOString(), mainThemes: themes, actions, clusters };

      if ((!out.actions || !out.actions.length) && (!out.mainThemes || !out.mainThemes.length)) {
        return fallbackDeterministic(articles, pool, topThemes);
      }
      return out;
    } catch {
