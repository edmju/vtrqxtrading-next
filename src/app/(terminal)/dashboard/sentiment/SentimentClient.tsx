...
function buildLine(points: number[], range: SparkRange) {
  ...
  let d = `M ${project(points[0], 0, n, range).x} ${project(points[0], 0, n, range).y}`;
  for (let i = 1; i < n; i++) {
...
function buildArea(points: number[], range: SparkRange) {
  ...
  let d = `M ${startX} ${baseY} L ${project(points[0], 0, n, range).x} ${project(points[0], 0, n, range).y}`;
  for (let i = 1; i < n; i++) {
    const p = project(points[i], i, n, range);
    d += ` L ${p.x} ${p.y}`;
  }
...
function buildTicks(ts: string[], maxTicks = 6) {
  ...
  const idx =
    n <= maxTicks
      ...
      : [0, Math.floor(n / 5), Math.floor((2 * n) / 5), Math.floor((3 * n) / 5), Math.floor((4 * n) / 5), n - 1];
  const multiDay = new Date(ts[0]).toDateString() !== new Date(ts[n - 1]).toDateString();
  return idx.map((i) => {
    const x = n === 1 ? 50 : (i * 100) / (n - 1);
    const d = new Date(ts[i]);
    const label = multiDay
      ? d.toLocaleString(undefined, { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit", hour12: false })
  ...
  return (
    <div className="relative w-full" style={{ height }}>
      <svg viewBox="0 0 100 40" preserveAspectRatio="none" className="absolute inset-0 h-full w-full">
        ...
        {/* fond + grille */}
        <rect x="0" y="0" width="100" height="40" fill="url(#nlc-area)" opacity="0.25" />
        {[0, 10, 20, 30, 40].map((y) => (
          <line key={y} x1="0" x2="100" y1={y} y2={y} stroke="rgba(148,163,184,0.15)" strokeWidth="0.25" />
        ))}

        {/* aire + ligne néon */}
        <path d={area} fill="url(#nlc-area)" opacity="0.55" />
        <path d={line} fill="none" stroke="url(#nlc-line)" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" filter="url(#nlc-glow)" />
...
export default function SentimentClient({ snapshot }: Props) {
  // Fenêtre d’affichage : 24h / 72h / 7j / Tout
  const [windowSel, setWindowSel] = useState<"24h" | "72h" | "7j" | "all">("all");

  const { globalScore, marketRegime, themes, riskIndicators, focusDrivers, history, suggestions } = snapshot;
  ...
  // Historique complet (AUCUN tronquage fixe)
  const fullHistory = useMemo<SentimentHistoryPoint[]>(
    () => (history || []).filter((h) => h && typeof h.globalScore === "number"),
    [history]
  );
  ...
  const windowedHistory = useMemo(() => {
    ...
    const map = { "24h": 24, "72h": 72, "7j": 7 * 24, all: Number.MAX_SAFE_INTEGER } as const;
        ...
        <div className="p-8 lg:p-10 space-y-10">
          <header className="flex flex-col md:flex-row md:items-end md:justify-between gap-5">
            <div className="space-y-1.5">
              <h1 className="text-3xl font-semibold text-neutral-50 tracking-tight">Sentiment de marché (vue IA)</h1>
              <p className="text-sm text-neutral-400">Lecture multi‑actifs construite à partir de nos flux d’actualités : forex, actions, commodities & régime global.</p>
            </div>
            <div className="flex flex-col items-start md:items-end gap-1 text-[12px]">
              <div className="text-neutral-400">Dernière mise à jour : <span className="text-neutral-200">{formattedDate}</span></div>
              {sourcesPresent && (
          ...
          <section className="grid gap-8 md:grid-cols-3">
            ...
            <div className="group rounded-3xl p-6 bg-gradient-to-br from-sky-900/50 via-sky-900/15 to-sky-600/10 border border-sky-500/35 shadow-[0_0_35px_rgba(8,47,73,0.9)] backdrop-blur-2xl transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_0_55px_rgba(8,47,73,1)] hover:border-sky-400/70">
              <div className="flex items-center justify-between">
                <div className="text-[11px] font-semibold uppercase tracking-wide text-sky-200/90">Global sentiment score</div>
              ...
              <div className="mt-3 space-y-1">
                <div className="flex items-center justify-between text-[11px] text-sky-100/80">
                  <span>{(snapshot.sourceConsensus ?? 50) >= 70 ? "Sources très alignées" : (snapshot.sourceConsensus ?? 50) >= 55 ? "Consensus correct" : (snapshot.sourceConsensus ?? 50) >= 40 ? "Lecture mitigée" : "Sources divergentes"}</span>
            ...
            <div className="group rounded-3xl p-6 bg-gradient-to-br from-violet-900/45 via-violet-900/20 to-violet-600/10 border border-violet-500/35 shadow-[0_0_35px_rgba(76,29,149,0.9)] backdrop-blur-2xl transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_0_55px_rgba(76,29,149,1)] hover:border-violet-400/70">
              <div className="flex items-center justify-between">
                ...
                <div className="text-[11px] text-neutral-100">Confiance : <span className="font-semibold">{marketRegime.confidence}/100</span></div>
              </div>
              <div className="mt-3 space-y-3">
                ...
                <div className="mt-1">
                  <div className="flex items-center justify-between text-[10px] text-neutral-300 mb-1">
                    <span>Risk‑off</span><span>Neutre</span><span>Risk‑on</span>
                  </div>
                  <div className="relative h-3 rounded-full bg-gradient-to-r from-rose-500/30 via-amber-400/20 to-emerald-500/30 border border-violet-400/30 overflow-hidden">
                    ...
                    <div className="absolute top-1/2 -translate-y-1/2 h-2.5 w-2.5 rounded-full bg-emerald-300 shadow-[0_0_12px_rgba(16,185,129,0.8)] animate-pulse" style={{ left: `calc(${Math.max(0, Math.min(100, globalScore))}% - 5px)` }} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="group rounded-3xl p-6 bg-gradient-to-br from-emerald-900/45 via-emerald-900/20 to-emerald-600/10 border border-emerald-500/35 shadow-[0_0_35px_rgba(6,95,70,0.9)] backdrop-blur-2xl transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_0_55px_rgba(6,95,70,1)] hover:border-emerald-400/70">
              <div className="flex items-center justify-between">
                ...
                {drivers.length > 0 && <div className="text-[11px] text-neutral-100">{drivers.length} axes</div>}
              </div>
              {drivers.length === 0 ? (
                <p className="mt-3 text-xs text-neutral-200">Pas de driver dominant détecté.</p>
              ) : (
                  ...
                  {drivers.map((d) => (
                    <li key={d.label} className="space-y-1">
                      <div className="flex items-center justify-between gap-2 text-xs">
                        ...
                        <span className="px-1.5 py-0.5 rounded-md bg-neutral-900/70 border border-neutral-700/70 text-neutral-200">Poids {d.weight}/3</span>
                      </div>
                      <div className="h-1.5 rounded-full bg-neutral-900 overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-emerald-400 via-lime-400 to-amber-300 transition-all duration-600" style={{ width: `${Math.max(10, d.widthPct)}%` }} />
                      </div>
                      {d.description && <p className="text-[11px] text-neutral-100 leading-snug">{d.description}</p>}
                    </li>
                  ))}
                </ul>
              )}
          ...
          <section className="space-y-4 pt-2 border-t border-neutral-900/80">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[11px] text-neutral-400">Sentiment global et par classe d’actifs sur la fenêtre sélectionnée.</p>
              </div>
            ...
            <div className="rounded-3xl border border-neutral-800/70 bg-neutral-950/90 backdrop-blur-2xl px-4 py-6">
              <div className="flex items-center justify-between text-[11px] text-neutral-300 mb-2">
                <span>Global</span>
                <span>Dernière valeur : <span className={scoreToColor(globalScore)}>{Math.round(globalScore)}/100</span></span>
              </div>
              <NeonLineChart points={windowedHistory.map(h => ({ t: h.timestamp, v: h.globalScore }))} height={200} />
            </div>
          ...
          <section className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] pt-6 border-t border-neutral-900/80">
            ...
            <div className="space-y-5">
              <div>
                ...
                <p className="text-[11px] text-neutral-400">Forex, actions et commodities – agrégés depuis nos flux.</p>
              </div>
                ...
                {[forex, stocks, commodities].filter(Boolean).map((t) => {
                  ...
                  return (
                    <div key={theme.id} className="space-y-1.5 transition-all duration-250 hover:bg-neutral-900/70 rounded-2xl px-3 py-2">
                      ...
                      {theme.comment && <div className="text-[11px] text-neutral-300">{theme.comment}</div>}
                    </div>
                  );
                })}
              </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="rounded-3xl border border-neutral-800/70 bg-neutral-950/88 backdrop-blur-2xl p-4 space-y-3">
                    {suggestions.slice(0, 3).map((s) => (
                      <div key={s.id} className="rounded-2xl px-3 py-3 bg-neutral-900/75 border border-neutral-700/70 flex flex-col gap-1.5 text-xs transition-all duration-250 hover:border-emerald-500/70 hover:bg-neutral-900 hover:-translate-y-0.5 hover:shadow-[0_0_20px_rgba(34,197,94,0.35)]">
                    ...
                    <div key={ind.id} className="space-y-1.5 transition-all duration-250 hover:bg-neutral-900/75 rounded-2xl px-3 py-2">
                      <div className="flex items-center justify-between gap-2 text-xs">
                        <div className="flex flex-col min-w-0">
                          ...
                          <span className="text-neutral-400 text-[11px]">{ind.value ?? "—"} · {dir}</span>
                        </div>
