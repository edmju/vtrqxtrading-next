// path: src/components/charts/NeonAreaChart.tsx
// (courbe fine, non lissée, brut – compat UI néon)
// Source d'origine: :contentReference[oaicite:0]{index=0}
"use client";
...
const NeonAreaChart: FC<Props> = ({ points, height = 200, showTicks = true }) => {
  ...
  return (
    <div className="relative w-full" style={{ height }}>
      <svg viewBox="0 0 100 40" preserveAspectRatio="none" className="absolute inset-0 h-full w-full" shapeRendering="crispEdges">
        <defs>
        ...
        {/* fond + grille */}
        <rect x="0" y="0" width="100" height="40" fill="url(#nlc-area)" opacity="0.18" />
        {[0, 10, 20, 30, 40].map((y) => (
          <line key={y} x1="0" x2="100" y1={y} y2={y} stroke="rgba(148,163,184,0.12)" strokeWidth="0.25" />
        ))}

        {/* aire + ligne (courbe fine, brute) */}
        <path d={area} fill="url(#nlc-area)" opacity="0.45" />
        <path d={line} fill="none" stroke="url(#nlc-line)" strokeWidth="1.1" strokeLinecap="square" strokeLinejoin="miter" />

        {/* point live minimal */}
        <rect x={lastProj.x - 0.6} y={lastProj.y - 0.6} width="1.2" height="1.2" fill="#22d3ee" />
      </svg>
